package likelion.festival.pub.repository;
...
public interface PubRepository extends JpaRepository<Pub, Long> {
    @Query("SELECT new likelion.festival.pub.dto.PubResponseDto(p.id, p.name ,p.likeCount) FROM Pub p ORDER BY p.likeCount DESC")
    List<PubResponseDto> findAllByOrderByLikeCountDesc();

    @Query("SELECT new likelion.festival.pub.dto.PubResponseDto(p.id, p.name, p.likeCount) FROM Pub p ORDER BY p.id")
